...
```php
	...
	<?php
		// Array Multidimensional – Arreglos dentro de un Arreglo
		$frutas = array(
			array('Manzana','rojo','12'),
			array('Naranja','naranjo','10'),
			array('Pera','verde','18')
		);
		echo $frutas[0][0].' Su color es:'.$frutas[0][1].' y quedan disponibles '.$frutas[0][2];
	?>
...
| # | Ejercicio | Concepto |
|---|-----------|----------|
| 1 | Determinar si un número es positivo, negativo o cero | Condicionales `if / elseif / else` |
| 2 | Función `suma($a, $b)` que retorna la suma de dos números | Funciones con parámetros y `return` |
| 3 | Función `calcularPromedio($numeros)` con un array de notas | Matrices y funciones |
| 4 | Imprimir números pares del 0 al 20 con `for`, `while` y `do-while` | Bucles |

> **Importante:** los valores se inicializan directamente en el código (sin validación de entrada de usuario).

### Ejercicio sincrónico – Sistema de calificaciones
...
**Requisitos:**
1. Define un array multidimensional con nombre y calificación de cada estudiante.
2. Usa una función para determinar si un estudiante aprueba (nota mayor o igual a 60).
3. Usa `foreach` para recorrer el array y `if` para verificar la condición.
4. Calcula el promedio del grupo con `array_sum()` y `count()`.
